...
void XSimple::isCheckNewestProtocol(bool flag)
{
    if (flag) {
...
#pragma mark 设置主队比分
void XSimple::setHomeScore(unsigned char score)
{
    m_dataProtocol[5] = score;
}

#pragma mark 设置客队比分
void XSimple::setGuestScore(unsigned char score)
{
    m_dataProtocol[7] = score;
}

#pragma mark 设置局数
void XSimple::setGameCount(unsigned char count)
{
    if (count > 5) {
        count = 5;
    }
...
#pragma mark 设置主队局分
void XSimple::setHomeGameScore(unsigned char score)
{
    m_dataProtocol[9] = score;
}

#pragma mark 设置客队局分
void XSimple::setGuestGameScore(unsigned char score)
{
    m_dataProtocol[10] = score;
}

#pragma mark 设置主队犯规数
void XSimple::setHomeFoulCount(unsigned char count)
{
    if (count > 9) {
        count = 9;
    }
...
#pragma mark 设置客队犯规数
void XSimple::setGuestFoulCount(unsigned char count)
{
    if (count > 9) {
        count = 9;
    }
...
#pragma mark 设置主队暂停数
void XSimple::setHomePauseCount(unsigned char count)
{
    if (count > 4) {
        count = 4;
    }
...
#pragma mark 设置客队暂停数
void XSimple::setGuestPauseCount(unsigned char count)
{
    if (count > 4) {
        count = 4;
    }
...
#pragma mark 设置24s
void XSimple::setAttackSecond(unsigned char second)
{
    m_dataProtocol[11] = second;
}

#pragma mark 设置分钟
void XSimple::setMinute(unsigned char minute)
{
    m_dataProtocol[12] = minute;
}

#pragma mark 设置秒
void XSimple::setSecond(unsigned char second)
{
    m_dataProtocol[13] = second;
}

#pragma mark 设置十分之一秒
void XSimple::set1_10Second(unsigned char second)
{
    m_dataProtocol[14] = second;
}

#pragma mark 设置球权:YES表示主队球权，NO表示客队球权
void XSimple::setBallControl(bool isHomeTeam)
{
    if (isHomeTeam) {
...
void XSimple::setDing(bool isDing)
{
    if (isDing) {
